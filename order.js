...
// [AI UPDATE 2026-09-19] Coupon rebuild — attach the customer-applied coupon (re-validated against the cart at this moment).
import { getCouponForOrder, clearAppliedAfterOrder } from "./coupons.js";
...
export async function placeOrder() {
  ...
  try {
    ...
    // totalPrice stays the PRE-coupon subtotal (POS re-prices from the items). The coupon is only a request:
    // the POS re-reads coupons/{code}, validates it, shows it to the manager, and marks it USED at settlement.
    const _coupon = getCouponForOrder(+totalPrice.toFixed(2));
    ...
    if (_coupon) clearAppliedAfterOrder();
