...
// [AI UPDATE 2026-09-19] Coupon rebuild — coupon state/eligibility comes from coupons.js (shared engine); this file only renders it.
import { getAppliedState, eligibleCoupons, applyCoupon, removeAppliedCoupon, onCouponsChange } from "./coupons.js";
...
export function initReview() {
  ...
  // [AI UPDATE 2026-09-19] Coupon apply/remove inside the existing order summary (no separate cart system).
  document.getElementById("reviewTotals")?.addEventListener("click", (e) => {
    const t = e.target.closest("[data-rv-apply],[data-rv-remove]");
    if (!t) return;
    if (t.dataset.rvRemove) removeAppliedCoupon();
    else applyCoupon(t.dataset.rvApply);
    _render();
  });
  // Coupons can change live (POS used one, new reward issued…) — keep the open sheet truthful.
  onCouponsChange(() => {
    if (!document.getElementById("reviewModal")?.classList.contains("hidden")) _render();
  });
}
...
function _render() {
  ...
  // [AI UPDATE 2026-09-19] Re-validated against the LIVE cart on every render: dropping below the coupon's minimum
  // immediately removes the discount (payable = subtotal), it can never be retained.
  const cpState = getAppliedState(totalAmt);
  ...
  totalsEl.innerHTML = `
    <div class="rv-summary">
      ...
      ${cpState.discount > 0 ? `
      <div class="rv-summary-row rv-coupon-row">
        <span class="rv-summary-label">🎟 Coupon (${_esc(cpState.coupon.code)})</span>
        <span class="rv-summary-value">−${fmt(cpState.discount)}</span>
      </div>` : ""}
      ${_couponBoxHtml(cpState, totalAmt, fmt)}
      <div class="rv-summary-divider"></div>
      <div class="rv-summary-grand">
        <span class="rv-summary-grand-label">Grand Total</span>
        <span class="rv-summary-grand-value">${fmt(cpState.payable)}</span>
      </div>
    </div>`;

  // Update sticky footer grand total
  const footerTotal = document.getElementById("reviewFooterTotal");
  if (footerTotal) footerTotal.textContent = fmt(cpState.payable);
...
/** Apply-coupon / applied / invalid box shown in the order summary. */
function _couponBoxHtml(st, subtotal, fmt) {
  if (st.discount > 0) {
    return `<div class="rv-coupon-box">🎟 <b>${_esc(discountLabel(st.coupon))}</b> applied · ${_esc(st.coupon.code)}
      <button data-rv-remove="1">Remove</button></div>`;
  }
  if (st.coupon && st.invalid) {
    return `<div class="rv-coupon-box rv-coupon-warn">⚠️ ${_esc(st.coupon.code)} not applied: ${_esc(st.message || "not valid for this cart")}
      <button data-rv-remove="1">Remove</button></div>`;
  }
  const el = eligibleCoupons(subtotal).slice(0, 3);
  if (!el.length) return "";
  return `<div class="rv-coupon-box">🎟 You have ${el.length === 1 ? "a coupon" : "coupons"} for this order:` +
    el.map(({ coupon, ev }) => `<div style="margin-top:6px;"><b>${_esc(discountLabel(coupon))}</b> · save ${fmt(ev.discount)}
      <button data-rv-apply="${_esc(coupon.code)}">Apply ${_esc(coupon.code)}</button></div>`).join("") + `</div>`;
}
